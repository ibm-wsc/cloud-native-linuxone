...
4. Fill the wizard with the parameters as shown in the image below (copy and paste boxes for the relevant parameters are below the image):

    !!! Note
        Your namespace will be different from the image below.
...
6. Verify your MySql instance is running
...
4. Now let's check the MySQL database to make sure that the new owner you just added is in there.
    ...
    1. Return to your OpenShift console, from the Topology view, click on the `mysql` icon. This will bring up a side panel, and then click on the `mysql` pod (your pod name will be different than the picture):

        ![mysql pod access](upandrunningimages/mysqlpod.png)

    2. In the pod panel, go to the Terminal tab.

        ![mysql terminal](upandrunningimages/mysqlterminal.png)

    3. Now type in the following commands in your `mysql` terminal (copy and paste box below image):
    ...
    4. Let's run a SQL command now to verify that the owner that you added through the application is indeed in the database (copy and paste box below image):
